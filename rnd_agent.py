...
class RNDAgent(DQNAgent):
    def __init__(
        ...
        **kwargs
        ...
        self.parameter_List.append(self.rnd_net.parameters())
    ...
    def update_rnd(self, obs: torch.Tensor) -> torch.Tensor:
        ...
        targetor = self.rnd_target_net
        targets = targetor(obs)
        predictor = self.rnd_net
        pred_targets = predictor(obs)
        loss = F.cross_entropy(pred_targets, targets)
    ...
    def update(
        ...
        with torch.no_grad():
            ...
            targetor = self.rnd_target_net
            targets = targetor(observations)
            predictor = self.rnd_net
            pred_targets = predictor(observations)
            rnd_error = (abs(pred_targets - targets))**2
